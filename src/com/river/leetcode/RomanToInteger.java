...
public class RomanToInteger {
   ...
   * <p>Example 1:
   *
   * <p>Input: s = "III" Output: 3 Example 2:
   *
   * <p>Input: s = "IV" Output: 4 Example 3:
   *
   * <p>Input: s = "IX" Output: 9 Example 4:
   *
   * <p>Input: s = "LVIII" Output: 58 Explanation: L = 50, V= 5, III = 3. Example 5:
   *
   * <p>Input: s = "MCMXCIV" Output: 1994 Explanation: M = 1000, CM = 900, XC = 90 and IV = 4.
   *
   * <p>Constraints:
   *
   * <p>1 <= s.length <= 15 s contains only the characters ('I', 'V', 'X', 'L', 'C', 'D', 'M'). It
   * is guaranteed that s is a valid roman numeral in the range [1, 3999].
   *
   * @param args
   */
  ...
  public static int romanToInt(String s) {
    ...
    for (int i = 0; i < s.length(); i++) {
      int r = convertToInt(s.charAt(i));
      result += r;
      if (i != 0) {
        int pre = convertToInt(s.charAt(i - 1));
        if (pre < r) {
          result -= (r + pre);
          result += (r - pre);
        }
